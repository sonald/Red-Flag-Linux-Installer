...
def fdhandler(dev,mem):
    ...
    if size > 10:
        ...
        disk = lib.rfparted.mkpart(dev, disk, parttype, start, end, 'linux-swap(v1)')
        start = end + 100
        if size > 30:
            end = parted.sizeToSectors(30, "GB", 512)
            disk = lib.rfparted.mkpart(dev, disk, parttype, start, end, "ext4")
            start = end + 100;
        end = sizeL - 100
        disk = lib.rfparted.mkpart(dev, disk, parttype, start, end, "ext4")
    elif size >= 6:
...
def easy_handler(self, dev, disk, parttype, start, end)
    ...
    if parttype == "free":
        if disk.primaryPartitionCount() == 4:
            ...
            disk = lib.rfparted.mkpart(dev, disk, "extended", start, end, fs)
            parttype = "logical"
        elif disk.primaryPartitionCount() < 4:
            parttype = "primary"
    disk = lib.rfparted.mkpart(dev, disk, "primary", start, end, fs)
    return disk
